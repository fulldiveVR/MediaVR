...
public class VreeTutorialScene extends TutorialScene {
    private String tag = null;

    public VreeTutorialScene(@NonNull final FulldiveContext fulldiveContext) {
    ...
    private void fillPage2(FrameLayout frameLayout) {
        ...
        addText(frameLayout, getString(R.string.mediavr_tutorial_photo_scroll_title), 1.3f, cx, top, 0xffffffff);

        imageControl = new ImageControl();
        imageControl.setPivot(.5f, .5f);
        imageControl.setSize(5f, 5f);
        imageControl.setPosition(cx, cy + 0.5f, 0f);
        imageControl.setImageBitmap(BitmapFactory.decodeResource(getResources(), R.drawable.mediavr_tutorial_photo_scroll));
        frameLayout.addControl(imageControl);

        addText(frameLayout, getString(R.string.mediavr_tutorial_photo_scroll_text), 0.9f, cx, cy + 3.5f, 0xffcccccc);
    }
}
